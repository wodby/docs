...
- `Protected` uses an [Application Access integration](../providers/access.md). The provider publishes the selected
  endpoints and Wodby suppresses the corresponding public routes.
...
1. `Access provider`: the integration that will publish and protect the app.
2. `Scope`: whether access covers the entire app or selected endpoints.
3. Provider-specific settings, such as a Cloudflare DNS zone and Access policy.
4. A primary hostname when the provider uses your own domain.
...
For example, a Drupal stack can receive one protected address for Drupal and another for Mailpit when both expose
external HTTP routes. Routes with different paths or aliases that target the same app port share one protected address.
...
For Cloudflare, select the DNS zone and then choose the `Flattened` or `Hierarchical` hostname structure. The form
previews the pattern and an example hostname that Wodby will use for additional services:
...
Tailscale assigns the hostname from the app and tailnet names, so no hostname field is shown.
...
persistent tunnel with stable hostnames in a customer-owned Cloudflare zone instead. See
...
address for each protected destination instead of the suppressed Wodby technical domain. Routes that target one app
...
| Provider | Protected access | Hostname |
| --- | --- | --- |
| [Cloudflare](../providers/cloudflare.md) | Cloudflare Tunnel plus a Cloudflare Access application and an existing reusable Access policy | A hostname in a selected Cloudflare DNS zone |
| [Tailscale](../providers/tailscale.md) | A private tailnet endpoint reachable by authorized Tailscale users and devices | Assigned from the app and tailnet names |
...
Cleanup only mutates resources Wodby still recognizes as its own. For example, if a Cloudflare DNS record was changed
after Wodby published it, Wodby leaves the record untouched and records a warning instead of deleting it or restoring
an older value. Customer-managed Cloudflare policies and certificates, and customer-managed Tailscale tags and access
