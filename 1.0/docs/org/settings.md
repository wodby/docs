...
New app instances inherit this default. Changing it also updates existing instances whose setting matches the previous organization default; instances with a different setting keep their override. To change one instance independently, open `Instance > Builds > Settings` and configure **Auto-void images of unused builds older than**.
...
See [Build image cleanup](../apps/builds.md#automatically-clean-unused-build-images) for details and manual cleanup, and [Container registry storage](../billing.md#container-registry-storage) for billing.
