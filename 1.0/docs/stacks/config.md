...
Protected environment values are hidden in stack configuration. You can replace
a protected value without seeing its current value.
...
Every successful reveal is recorded in the instance task history, including who
revealed which variable and when. The history does not contain the secret. Users
who can update the instance can view its reveal history; entries are retained
for one year.
